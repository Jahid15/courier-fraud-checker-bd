...
$config = [
    'pathao' => [
        'user' => 'ibnasinha15@gmail.com',        // Replace with your Pathao email
        'password' => 'P@ssword0194',             // Replace with your Pathao password
    ],
    'redx' => [
        'phone' => '01676565090',                         // Replace with your RedX phone
        'password' => 'P@ssword0194',               // Replace with your RedX password
    ],
    'steadfast' => [
        'user' => 'ibnasinha15@gmail.com',     // Replace with your Steadfast email
        'password' => 'P@ssword0194',          // Replace with your Steadfast password
    ],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['phoneNumber'])) {
    // Hard-reset output buffering and force JSON
    ...
    try {
        ...
        if ($demoMode) {
            ...
            // Use your existing cached() wrapper around direct service functions
            $results = [
                'steadfast' => cached('steadfast:' . $phoneNumber, 300, function() use ($phoneNumber, $config) {
                    return checkSteadfast($phoneNumber, $config['steadfast']['user'], $config['steadfast']['password']);
                }),
                'pathao' => cached('pathao:' . $phoneNumber, 300, function() use ($phoneNumber, $config) {
                    return checkPathao($phoneNumber, $config['pathao']['user'], $config['pathao']['password']);
                }),
                'redx' => cached('redx:' . $phoneNumber, 300, function() use ($phoneNumber, $config) {
                    return checkRedx($phoneNumber, $config['redx']['phone'], $config['redx']['password']);
                }),
            ];
        }
